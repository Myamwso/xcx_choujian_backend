...
require_once __DIR__ . "/class/scoreCommonCode/Score.class.php";
...
//    require_once __DIR__ . "/wxapp_linsd.php";
//
//    $action = "doPage".$_GPC['do'];
//    switch ($_GPC['v']) {
//        case 'linsd':
//            $class = "Choujiang_pageModuleWxapp_linsd";
//            $obj = new $class();
//            $obj->$action();
//            exit;
//            break;
...
///当前版本v7
class Choujiang_pageModuleWxapp_v8 extends WeModuleWxapp
{
    protected $attachurl;
...
//        }
        if ($item['type'] == 1)   //阿里云oss 开启
        {
            ...
            if ( isset( $_GPC['type']) && $_GPC['type'] == "wishing" ) {
                $destination_folder = '../attachment/wishing/' . $_W['uniacid'] . '/' . date('Y/m/') . '/';  //图片文件夹路径
            }
            ...
            if ( isset( $_GPC['type']) && $_GPC['type'] == "wishing" ) {
                $filename = 'wishing/' . $_W['uniacid'] . '/' . date('Y/m/') . $pinfo['basename'];
            }
            ...
            try {
                ...
                return error(1, json_encode($e->getMessage()));
...
//                var_dump($data);
    ...
    // 参与抽奖
    public function doPageParticipate( $is_return = 0 )
    {
        global $_W, $_GPC;
        $uniacid = $_W['uniacid'];
        $openid = $_REQUEST['openid'];
