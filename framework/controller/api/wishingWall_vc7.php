...
class Cj_wishingWall_vc7 extends Common
{
...
//        }
        ...
        ///写入心愿表
//        $wishingInsert = pdo_insert_cj("choujiang_wishing", $data);
        ...
        if($wishingInsert){
            /// 给心愿发布者发放积分
            $this->score->addScore($openid, $this->baseConfig['score']['release_wishing'], ["getInof"=>"发布【{$goodsName}】，id为{$wishingId}"]);
    ...
    public function editWishing(){
        ...
        ///获取心愿详情
//        $wishingInfo = pdo_get_cj("choujiang_wishing",['id' => $wishingId]);
        ...
        ///写入心愿表
//        $wishingUpdate = pdo_update_cj("choujiang_wishing", $data, ['id' => $wishingId]);
    ...
    public function wishingList(){
        ...
        //$wishingList = pdo_fetchall_cj("SELECT * FROM " . tablename_cj('choujiang_wishing') . " WHERE release_goods = 0 AND create_at<= ORDER BY likes_num DESC, id ASC LIMIT  " . ($pindex - 1) * $psize . "," . $psize);
//        $total = pdo_fetch_cj("SELECT count(*) AS count FROM " . tablename_cj('choujiang_wishing') . " WHERE release_goods = 0 AND create_at>='".date("Y-m-d H:i:s",mktime(0, 0 , 0,date("m"),date("d")-date("N")+1,date("Y")))."' AND create_at<='".date("Y-m-d H:i:s",mktime(23,59,59,date("m"),date("d")-date("N")+7,date("Y"))) . "'");
//        $wishingList = pdo_fetchall_cj("SELECT * FROM " . tablename_cj('choujiang_wishing') . " WHERE release_goods = 0 AND create_at>='".date("Y-m-d H:i:s",mktime(0, 0 , 0,date("m"),date("d")-date("N")+1,date("Y")))."' AND create_at<='".date("Y-m-d H:i:s",mktime(23,59,59,date("m"),date("d")-date("N")+7,date("Y")))."' ORDER BY openid<>'{$openid}' ASC, likes_num DESC, id ASC LIMIT  " . ($pindex - 1) * $psize . "," . $psize);
...
//            $wishingList['list'][$key]['nickname'] = empty($userInfo) ? "" : $userInfo['nickname'] ;
//            $wishingList['list'][$key]['likes_id'] = empty($wishingJion) ? 0 : $wishingJion['id'] ;
//
//        }
...
//            'openid !=' => $openid,
            ...
            'create_at >=' => $weekStart,
            'create_at <=' => $weekEnd,
        ], [], '', ["release_goods<>1 ASC", "likes_num DESC"],[$pindex, $psize]);
...
//                    return $this->result(0, 'success', ['error'=>1,'message'=>'您已经达到本周发布心愿上限']);
     ...
     * 更新自动参与抽奖formid
     */
    private function editAutoJoinFormid() {
        global $_GPC;
